...
int main(void) {
    ...
    for (int y = 0; y < size; y++) {
        ...
        for (int x = 0; x < size; x++) {
            double noise_x = x * scale;
            double noise_y = y * scale;
            double fbm = simplex_fbm_2d(noise_x, noise_y, config.octaves, config.persistence,
                                        config.lacunarity);
            char char_fbm = fbm > 0.5 ? '#' : fbm > 0 ? '+' : fbm > -0.5 ? '.' : '-';
            printf("%c", char_fbm);
        }
        printf("  ");  // Separator
        ...
        for (int x = 0; x < size; x++) {
            double noise_x = x * scale;
            double noise_y = y * scale;
            double hybrid = simplex_hybrid_multifractal_2d(
                noise_x, noise_y, config.octaves, config.persistence, config.lacunarity, 0.3);
            char char_hybrid = hybrid > 0.5 ? '#' : hybrid > 0 ? '+' : hybrid > -0.5 ? '.' : '-';
