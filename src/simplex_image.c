...
static int write_ppm_pixels(FILE *file, const uint8_t *pixels, int width, int height, int channels) {
...
static int write_png_simple(FILE *file, const uint8_t *pixels, int width, int height, int channels) {
    ...
    return write_ppm_header(file, width, height, 255) > 0 && 
           write_ppm_pixels(file, pixels, width, height, channels) == 0 ? 0 : -1;
}
...
static int convert_to_pixels(const double *noise_data, uint8_t *pixels, 
                            int width, int height, simplex_color_mode_t color_mode) {
    int channels = (color_mode == SIMPLEX_COLOR_GRAYSCALE || color_mode == SIMPLEX_COLOR_HEIGHTMAP) ? 1 : 3;
        ...
        for (int x = 0; x < width; x++) {
            ...
            switch (color_mode) {
                ...
                case SIMPLEX_COLOR_RGBA:
                    noise_to_rgb(noise, &pixels[pixel_index], 
                               &pixels[pixel_index + 1], &pixels[pixel_index + 2]);
                    break;
                    
                case SIMPLEX_COLOR_HEIGHTMAP:
                    noise_to_heightmap(noise, &pixels[pixel_index], 
                                     &pixels[pixel_index + 1], &pixels[pixel_index + 2]);
                    break;
                    
                case SIMPLEX_COLOR_TERRAIN:
                    noise_to_terrain(noise, &pixels[pixel_index], 
                                   &pixels[pixel_index + 1], &pixels[pixel_index + 2]);
                    break;
        ...
        case SIMPLEX_IMAGE_PPM:
            if (channels == 1) {
                result = write_pgm_header(file, config->width, config->height, 255) > 0 &&
                        write_pgm_pixels(file, pixels, config->width, config->height) == 0 ? 0 : -1;
            } else {
                result = write_ppm_header(file, config->width, config->height, 255) > 0 &&
                        write_ppm_pixels(file, pixels, config->width, config->height, channels) == 0 ? 0 : -1;
            }
            break;
...
int simplex_generate_image_series(const simplex_image_config_t *base_config, int count,
    ...
    if (!base_config || count <= 0) return -1;
...
int simplex_generate_animation(const simplex_image_config_t *config, int frame_count,
    ...
    if (!config || frame_count <= 0 || !output_dir) return -1;
